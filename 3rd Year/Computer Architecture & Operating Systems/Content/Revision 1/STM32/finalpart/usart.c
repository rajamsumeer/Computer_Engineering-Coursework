...
// Configure PA2 & PA15 to Alternate Function
//PA2 (TX)
GPIOA->MODER &= ~(GPIO_MODER_MODER2);//(0x3 << 4); // Clear mode bits
GPIOA->MODER |= (GPIO_MODER_MODER2_1);//(0x2 << 4); // Set AF (10)
//PA15 (RX)
...
// AF7 for USART2
//PA2
GPIOA->AFR[0] &= ~(GPIO_AFRL_AFRL2);//(0xF << 8); // Clear bits
GPIOA->AFR[0] |= (0x7 << GPIO_AFRL_AFRL2_Pos);//(0x7 << 8); // Set AF7
//PA15
GPIOA->AFR[1] &= ~(GPIO_AFRH_AFRH7);//(0xF << 28);
GPIOA->AFR[1] |= (0x7 << GPIO_AFRH_AFRH7_Pos);//(0x7 << 8);

USART2->BRR = 833; // Configure Baud Rate for 9600 (8Mhz/9600)
